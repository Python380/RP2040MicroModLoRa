...
void LoRaClass::setRfSwitchPins(uint8_t rx, uint8_t tx) {
  LoRaClass::useRfSwitchPins = true;
  LoRaClass::rfSwitchRx = rx;
  LoRaClass::rfSwitchTx = tx;
  gpio_init(rx);
  gpio_set_dir(rx, GPIO_OUT);
  gpio_init(tx);
  gpio_set_dir(tx, GPIO_OUT);
  LoRaClass::setRfSwitchState(false, false);
}

void LoRaClass::setRfSwitchState(bool rx, bool tx) {
